...
try:
    ...
    if graph_oecd_status_abs is None:
...
def _enrichment_background_size() -> int:
    """Total HGNC genes mapped to any KE in the latest graph (denominator)."""
    q = """
    ...
    LIMIT 1
    """
    # Fallback simple count across the latest graph only.
    try:
        latest_q = (
            "SELECT ?graph WHERE { GRAPH ?graph { ?s a aopo:AdverseOutcomePathway } "
            "FILTER(STRSTARTS(STR(?graph), \"http://aopwiki.org/graph/\")) } "
            "GROUP BY ?graph ORDER BY DESC(?graph) LIMIT 1"
        )
        latest = run_sparql_query(latest_q)
        if not latest:
            return 0
        graph_uri = latest[0]['graph']['value']
        bg = run_sparql_query(
            f"SELECT (COUNT(DISTINCT ?g) AS ?n) WHERE {{ "
            f"GRAPH <{graph_uri}> {{ ?ke a aopo:KeyEvent ; "
            f"<http://edamontology.org/data_1025> ?g }} }}"
        )
        return int(bg[0]['n']['value']) if bg else 0
    except Exception:
        return 0
...
def enrichment():
    """Toxicologist gene-list → AOP hypergeometric enrichment (#63)."""
    if request.method == "GET":
        bg = _enrichment_background_size()
    ...
    raw = (request.form.get('genes') or '').strip()
    if not raw:
        return render_template("enrichment.html", active_page='enrichment',
                               error="Paste at least one HGNC gene symbol.",
                               raw_input=raw,
                               background_size=_enrichment_background_size())

    # Parse: split on any whitespace / commas / semicolons.
    import re
    symbols = [s.strip().upper() for s in re.split(r'[\s,;]+', raw) if s.strip()]
    symbols = list(dict.fromkeys(symbols))  # preserve order, dedupe
    if not symbols:
        return render_template("enrichment.html", active_page='enrichment',
                               error="Could not parse any gene symbols.",
                               raw_input=raw,
                               background_size=_enrichment_background_size())

    # Resolve latest graph.
    latest_q = (
        "SELECT ?graph WHERE { GRAPH ?graph { ?s a aopo:AdverseOutcomePathway } "
        "FILTER(STRSTARTS(STR(?graph), \"http://aopwiki.org/graph/\")) } "
        "GROUP BY ?graph ORDER BY DESC(?graph) LIMIT 1"
    ...
    if not latest:
        return render_template("enrichment.html", active_page='enrichment',
                               error="Could not resolve the latest snapshot.",
                               raw_input=raw,
                               background_size=0)
    graph_uri = latest[0]['graph']['value']

    # Resolve symbols → HGNC URIs in the latest graph.
    values_block = ' '.join(f'"{s}"' for s in symbols)
    resolve_q = f"""
    SELECT DISTINCT ?gene ?label WHERE {{
      GRAPH <{graph_uri}> {{
        ?gene <http://www.w3.org/2000/01/rdf-schema#label> ?label .
        ?gene a <http://edamontology.org/data_1025> .
        FILTER(UCASE(STR(?label)) IN ({', '.join(f'"{s}"' for s in symbols)}))
      }}
    }}
    """
    try:
        gene_rows = run_sparql_query(resolve_q)
    except Exception as e:
        return render_template("enrichment.html", active_page='enrichment',
                               error=f"SPARQL error resolving symbols: {e}",
                               raw_input=raw,
                               background_size=0)
    ...
    matched_labels = set()
    for r in gene_rows:
        matched_uris.append(r['gene']['value'])
        matched_labels.add(r.get('label', {}).get('value', '').upper())
    unmatched = [s for s in symbols if s not in matched_labels]

    if not matched_uris:
        return render_template(
            "enrichment.html", active_page='enrichment',
            results=[],
            unmatched=unmatched,
            matched_count=0,
    ...
    # Background size (denominator).
    bg = _enrichment_background_size()

    # Per-AOP query: #KE hits + #gene hits + total KEs + total genes.
    gene_values = ' '.join(f'<{u}>' for u in matched_uris)
    aop_q = f"""
    SELECT ?aop ?title
           (COUNT(DISTINCT ?ke) AS ?ke_hits)
           (COUNT(DISTINCT ?gene_hit) AS ?gene_hits)
    WHERE {{
      GRAPH <{graph_uri}> {{
        ?aop a aopo:AdverseOutcomePathway ;
             aopo:has_key_event ?ke .
        ?ke <http://edamontology.org/data_1025> ?gene_hit .
        OPTIONAL {{ ?aop <http://purl.org/dc/elements/1.1/title> ?title }}
        VALUES ?gene_hit {{ {gene_values} }}
      }}
    }}
    GROUP BY ?aop ?title
    ORDER BY DESC(?ke_hits)
    """
    hits = run_sparql_query(aop_q)
    if not hits:
        return render_template(
            "enrichment.html", active_page='enrichment',
            results=[],
    ...
    # Per-AOP totals (denominators for hypergeometric).
    totals_q = f"""
    SELECT ?aop (COUNT(DISTINCT ?ke) AS ?total_kes) (COUNT(DISTINCT ?g) AS ?total_genes)
    WHERE {{
      GRAPH <{graph_uri}> {{
        ?aop a aopo:AdverseOutcomePathway ;
             aopo:has_key_event ?ke .
        OPTIONAL {{ ?ke <http://edamontology.org/data_1025> ?g }}
        VALUES ?aop {{ {' '.join(f'<{h["aop"]["value"]}>' for h in hits)} }}
      }}
    }}
    GROUP BY ?aop
    """
    totals_rows = run_sparql_query(totals_q)
    totals_by_aop = {
        r['aop']['value']: (int(r.get('total_kes', {}).get('value', 0)),
                            int(r.get('total_genes', {}).get('value', 0)))
        for r in totals_rows
    }

    # Hypergeometric: sf(k-1, M, n, N) for one-sided enrichment.
    # M = bg (background gene pool)
    # n = gene_hits (user's matched genes in AOP-Wiki) is fixed; we use len(matched_uris)
    # N = total_genes in the AOP
    # k = gene_hits (in this AOP)
    from scipy.stats import hypergeom  # local import — keeps cold start light
    n_user = len(matched_uris)
    results = []
    for h in hits:
        aop_uri = h['aop']['value']
        aop_id = aop_uri.rsplit('/', 1)[-1]
        title = h.get('title', {}).get('value', '—')
        ke_hits = int(h.get('ke_hits', {}).get('value', 0))
        gene_hits = int(h.get('gene_hits', {}).get('value', 0))
        total_kes, total_aop_genes = totals_by_aop.get(aop_uri, (0, 0))
        # P(X >= gene_hits): sf(k-1)
        if bg > 0 and total_aop_genes > 0 and n_user > 0:
            pval = float(hypergeom.sf(gene_hits - 1, bg, total_aop_genes, n_user))
        else:
            pval = 1.0
        results.append({
            'aop_id': aop_id,
            ...
            'total_kes': total_kes,
            'gene_hits': gene_hits,
            'total_aop_genes': total_aop_genes,
            'pvalue': pval,
        })
    ...
    m = len(results)
    fdrs = [r['pvalue'] * m / (i + 1) for i, r in enumerate(results)]
    # Monotone enforcement, BH style.
    for i in range(m - 2, -1, -1):
        fdrs[i] = min(fdrs[i], fdrs[i + 1])
    for r, f in zip(results, fdrs):
        r['fdr'] = min(1.0, f)
        r['pvalue_str'] = _format_pvalue(r['pvalue'])
        r['fdr_str'] = _format_pvalue(r['fdr'])
