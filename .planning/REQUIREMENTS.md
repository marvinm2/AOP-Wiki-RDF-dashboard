...
- [x] **NETW-01**: User can explore interactive AOP network graph showing AOPs connected through shared KEs (click nodes, filter by properties, zoom)
- [x] **NETW-02**: User can view centrality metrics (degree, betweenness, closeness) for Key Events displayed as sortable table and highlighted on network graph
- [x] **NETW-03**: User can view PageRank scores ranking Key Events by structural importance in the AOP network
- [x] **NETW-04**: User can see community/cluster groupings of related AOPs with cluster visualization on network graph
